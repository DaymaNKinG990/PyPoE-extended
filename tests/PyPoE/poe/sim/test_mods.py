...
class TestGetSpawnChanceCalculator:
    ...
    def test_total_weight(self, scc):
        assert scc["default"].get_total_spawn_weight() == 5000
        assert scc["half"].get_total_spawn_weight() == 4500
        assert scc["nope"].get_total_spawn_weight() == 4000

    def test_spawn_chance_no_remove(self, scc):
        assert scc["default"].spawn_chance("0", remove=False) == 1000 / 5000
        assert scc["half"].spawn_chance("0", remove=False) == 1000 / 4500
        assert scc["half"].spawn_chance("4", remove=False) == 500 / 4500

    def test_spawn_chance_remove(self, scc):
        assert scc["default"].spawn_chance("0", remove=True) == 1000 / 5000
        assert scc["default"].spawn_chance("3", remove=True) == 1000 / 2000
        assert scc["default"].spawn_chance("0", remove=False) == 0
...
class TestGetSpawnWeight:
    ...
    def test_order(self):
        ...
        assert mods.get_spawn_weight(self.data, ["default", "a"]) == 1
        assert mods.get_spawn_weight(self.data, ["b", "c", "default"]) == 2
