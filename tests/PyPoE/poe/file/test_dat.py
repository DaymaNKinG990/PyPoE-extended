...
class TestDatValue:
    ...
    def test_instance_basic(self):
        ...
        assert not self.dv_basic.is_list, "Should not be a list"
        assert self.dv_basic.is_parsed, "Should count as parsed value"
    ...
    def test_instance_raw(self):
        ...
        assert not self.dv_raw.is_list, "Should not be a list"
        assert not self.dv_raw.is_parsed, "Should count as unparsed/binary value"
    ...
    def test_instance_pointer(self):
        ...
        # Property Tests
        assert self.dv_pointer.data_size == self.dv_pointer.child.size, (
            "Should return the size of the child"
        )
        assert self.dv_pointer.data_start_offset == self.dv_pointer.value, (
        ...
        assert not self.dv_pointer.is_data, "Should not be data"
        assert self.dv_pointer.has_data, "Should have data"
        assert self.dv_pointer.is_pointer, "Should be a pointer"
        assert not self.dv_pointer.is_list, "Should not be a list"
        assert self.dv_pointer.is_parsed, "Should count as parsed value"
        ...
        assert not self.dv_pointer.child.is_list, "Should not be a list"
        assert self.dv_pointer.child.is_parsed, "Should count as parsed value"
    ...
    def test_instance_list(self):
        ...
        # Property Tests
        assert self.dv_list.data_size == self.dv_list.value[0] * self.dv_list.children[0].size, (
            "Should return sizeof(self) + sizeof(child)"
        )
        assert self.dv_list.data_start_offset == self.dv_list.value[1], (
            "Should return the self.dv_list.value, i.e. the pointer"
        )
        assert self.dv_list.data_end_offset == self.dv_list.value[1] + self.dv_list.data_size, (
            "Should be the pointer + sizeof(child)"
        )
        assert not self.dv_list.is_data, "Should not be data"
        assert self.dv_list.has_data, "Should have data"
        assert not self.dv_list.is_pointer, "Should not be a pointer"
        assert self.dv_list.is_list, "Should be a list"
        assert self.dv_list.is_parsed, "Should count as parsed value"
        ...
        for i in range(0, len(self.dv_list.children)):
            ...
            assert not child.is_list, "Should not be a list"
            assert child.is_parsed, "Should count as parsed value"
...
def test_dat_file(testspec_dat_file):
    ...
    for row in dr:
        ...
        list_value = row["ref|list|int"]
        assert list_value[0] == test_list[0], "Value mismatch - list"
        assert list_value[1] == test_list[1], "Value mismatch - list"
        # 0xFEFEFEFE is a magic key, so return -1
        assert list_value[2] is None, "Value mismatch - list, special value"
